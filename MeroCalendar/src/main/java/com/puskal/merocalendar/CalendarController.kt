...
object CalendarController {
    ...
    fun getDateList(
        ...
        return when (calendarType) {
            CalendarType.AD -> englishCalendarController(localizationType)
    ...
    fun englishCalendarController(localizationType: LocalizationType): Pair<ArrayList<DateModel>,String> {
      val calendar=Calendar.getInstance()
        ...
        Log.d(
            "mero",
            "liek me d${LocalizationHelper.englishMonthNameInEnglishFont(calendar.get(Calendar.MONTH).plus(1))}, ${
                calendar.get(
                    Calendar.YEAR
                )
            }"
        ...
        val monthYearTitle = when (localizationType) {
            LocalizationType.ENGLISH_US->{
                ...
                LocalizationHelper.toNepaliDigit("${LocalizationHelper.englishMonthNameInNepaliFont(monthNum)}, $year")
            }
        }
        ...
        for (i in 1..daysInThisMonth) {
            ...
            val dispDay=if(localizationType==LocalizationType.ENGLISH_US) i.toString() else LocalizationHelper.toNepaliDigit(i.toString())
            val dateModel =
                DateModel(
                    dispDay,
                    adDate = date,
